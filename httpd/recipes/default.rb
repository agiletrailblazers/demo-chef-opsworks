...
unless platform_family?('debian')
  ...
  include_recipe 'httpd::mod_deflate'
end
...
if node['apache']['version'] == '2.4' && !platform_family?('freebsd')
  ...
  include_recipe "httpd::mpm_#{node['apache']['mpm']}"
end

node['apache']['default_modules'].each do |mod|
  module_recipe_name = mod =~ /^mod_/ ? mod : "mod_#{mod}"
  include_recipe "httpd::#{module_recipe_name}"
end
...
httpd_site node['apache']['default_site_name'] do
  enable node['apache']['default_site_enabled']
end
